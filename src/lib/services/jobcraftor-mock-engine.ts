...
function getKeywordRecommendations(jobPosting: string, resume: string, matchedSkills: string[], gaps: GapItem[]) {
  ...
  const fromSignals = roleSignals
    .filter((signal) => !matchedSkills.includes(signal.title) && hasSignal(cleanJob, signal.aliases))
    .map((signal) => signal.title);
...
export function generateMockJobCraftorAnalysis(input: AnalyzeJobCraftorInput): JobCraftorResult {
